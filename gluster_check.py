...
parser.add_argument("-n", "--name", help="Provide the gluster volumne name to check")
...
class GlusterStats(object):
    ...
    def _format_stats(self):
        ...
        out['gluster_stats_version'] = script_version
...
if args.name:
    try:
        stats = GlusterStats(timeout=30)
        stats_data = json.dumps(stats.get_stats(), indent=4, sort_keys=True)
        stats_json = Load(stats_data)

        count=0
        for x in stats_json.brick_stats[args.name]:
            if stats_json.brick_stats[args.name][x]['online'] == 1:
                count +=1
        print count
    except:
        print("exception!")
else:
    print("No gluster volume name provided, use the -n flag")
